	...
	public class ClayContainer : MonoBehaviour{
		...
		*/
		...
		static public void initGlobalData(){
			...
			ClayContainer.lastExpandedContainerId = -1;
		...
		public void init(){
			...
			if(!this.interactive && !this.editingThisContainer){
				this.optimizeMemory();
			}
		...
		*/
		...
		/* ClayContainer.generateMesh() can optionally retain some buffers to speedup multiple calls to generateMesh (freeMemory = false), 
		...
		public Mesh generateMesh(int detail, bool colorizeMesh = false, bool computeNormals = false, float smoothNormalAngle = 100.0f, bool freeMemory = true){
			...
			if(mesh != null){
				...
				if(computeNormals){
					MeshUtils.smoothNormals(mesh, smoothNormalAngle, this.gameObject);
				}
			}
		...
		public void freezeToMesh(int meshDetail, float smoothAngle = 100.0f, bool freeMemory = true){
			...
			if(mat == null){
				if(ClayContainer.renderPipe == "hdrp"){
					...
					render.sharedMaterial = new Material(Shader.Find("Clayxels/ClayxelHDRPMeshShaderASE" + renderModeMatSuffix));
				}
					...
					render.sharedMaterial = new Material(Shader.Find("Clayxels/ClayxelURPMeshShaderASE" + renderModeMatSuffix));
				}
				else{
				...
				Texture texture = render.sharedMaterial.GetTexture("_MainTex");
				if(texture == null){
					if(this.renderMode == ClayContainer.RenderModes.microVoxel){
						render.sharedMaterial.SetTexture("_MainTex", (Texture)Resources.Load("clayxelDotBlur"));
					}
					else{
						render.sharedMaterial.SetTexture("_MainTex", (Texture)Resources.Load("clayxelDot"));
					}
				}
			}
		...
		public void smoothNormalsContainersHierarchy(float smoothNormalAngle){
			...
			for(int i = 0; i < containers.Length; ++i){
				...
				if(container.isFrozenToMesh()){
					...
					MeshUtils.smoothNormals(container.GetComponent<MeshFilter>().sharedMesh, smoothNormalAngle, container.gameObject);
				}
			}
		}
		...
		*/
		...
		static int lastExpandedContainerId = -1;
		...
		void optimizeMemorySmoothMesh(){
			...
			if(numElements == 0){
				...
				this.numPointsInChunkBuffer.Release();
				this.compBuffers.Remove(this.numPointsInChunkBuffer);
				this.numPointsInChunkBuffer = null;
		...
		void switchMicrovoxelSharedMemory(){
			if(!this.interactive){
				if(ClayContainer.containersInScene.ContainsKey(ClayContainer.lastExpandedContainerId) && ClayContainer.lastExpandedContainerId != this.containerId){
					ClayContainer lastEditedContainer = ClayContainer.containersInScene[ClayContainer.lastExpandedContainerId];
				...
				ClayContainer.lastExpandedContainerId = this.containerId;
			}
		}
